...
import random
# from imbalanced_loss import logistic_loss, sigmoid_loss
...
class AUPRCSampler(Sampler):
    ...
    def __iter__(self):
        ...
        if len(minority_data_list) // self.posNum >= len(majority_data_list) // (
            ...
            self.batchSize - self.posNum) + 1 - len(minority_data_list) // self.posNum, replace=True).tolist())
            for i in range(0, len(majority_data_list), self.batchSize - self.posNum):
                ...
                if self.posNum == 1:
                    ...
                    self.ret.extend(minority_data_list[i // (self.batchSize - self.posNum) * self.posNum: (i // (
                    self.batchSize - self.posNum) + 1) * self.posNum])
...
class SOAPLOSS(nn.Module):
    def __init__(self, threshold, data_length, loss_type='sqh', gamma = 0.9):
        ...
        self.u_all = 1.0 * torch.tensor([0] * data_length).view(-1, 1).cuda()
